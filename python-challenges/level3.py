...
big_str = "".join(x.strip().split("\n"))



for i in range(4, len(big_str)-4):
    val = ord(big_str[i])
    if val >= ord('a') and val <= ord('z'):
        count = 0
        for char in big_str[i-3:i]:
        ...
        tip_l = ord(big_str[i - 4])
        tip_r = ord(big_str[i + 4])
        if tip_l >= ord('a') and tip_l <= ord('z') and tip_r >= ord('a') and tip_r <= ord('z'):
            print(big_str[i], end="")
